...
-module(egrpc_grpc_reflection_v1_server_reflection_service).

-export([server_reflection_info/1, server_reflection_info/2]).

-define(GRPC_SERVICE_NAME, 'grpc.reflection.v1.ServerReflection').

%% @doc Call the bidi_streaming RPC method `ServerReflectionInfo' of service `grpc.reflection.v1.ServerReflection'.
...
server_reflection_info(Channel, Opts) ->
    RpcDef = egrpc_reflection_v1_pb:find_rpc_def(?GRPC_SERVICE_NAME, 'ServerReflectionInfo'),
    Grpc = #{
        rpc_def => RpcDef,
        path => <<"/grpc.reflection.v1.ServerReflection/ServerReflectionInfo">>,
        pb_module => egrpc_reflection_v1_pb,
